var interactiveViewing = {
	...
	leftClick: function(elem, JOBADInstance) {
	   ...
	   //handling clicks on parts of the document
		...
		if(target.hasAttribute('data-mmt-flattenable')) {
			...
			var fc = jQuery(elem).children().filterMClass('flat-container');
			fc.children().replaceWith(loaded);
			fc.toggle();
		}
		if(target.hasAttribute('foldable')) {
			var content = jQuery(target).parent().find('table').toggle();				
		}
		...
		if (mmt.currentURI !== null) {
      	...
      	var name = mmt.splitMMTURI(uri, false).slice(-1)[0];
			...
			res["show graph"] = function() {
				...
				jQuery(contentNode).addClass('graph-body');
      ...
      qmt.exec(query,
   	   function(result){
   		  try {
			    ...
			    jQuery(contentNode).append(pres);
   		  } catch (err) {
			    var errorNode = inlineBox.create(mmt.target, "error");
			    jQuery(errorNode).append(result.firstChild);
   		  }
   	   }
   	);
	},
	...
	addComment: function() {
	    ...
	    jQuery(first).append("<form id=\"form-id\" method=\"post\">" +
	    ...
	    jQuery(commNode).append(first);
	    mmt.ajaxAppendBox(query, commNode);
	    jQuery(first).find("#btn-id").click(function() {
	    	var data = {
	        user: jQuery(first).find("#user-id").val(),
	        comment: jQuery(first).find("#comm-id").val()
	        }
	        var toSend = JSON.stringify(data);
	    	jQuery(first).find("#form-id").submit(function() {
	        ...
	        jQuery.ajax({
                ...
                success: function(data) {
                    ...
                    jQuery(first).replaceWith(replace)
                		}
            		});
	        return false;
	        	});
		...
		jQuery.ajax({
	        ...
	        success: function(data) {
	        	if(data.length === 0) {
	            	...
	            	jQuery(alignNode).append(hol)
	            	jQuery(alignNode).append(miz)		                	 
	            	}   	
	        	}
	   		});
		},
